...
class TestDocumentBlock:
    ...
    def test_page_must_be_positive(self) -> None:
        with pytest.raises(ValidationError):
            DocumentBlock(
                block_id="0_0", page=0, text="x", block_type=BlockType.PARAGRAPH, confidence=0.9
            )

    def test_confidence_bounds(self) -> None:
        DocumentBlock(
            block_id="1_0", page=1, text="x", block_type=BlockType.PARAGRAPH, confidence=0.0
        )
        DocumentBlock(
            block_id="1_0", page=1, text="x", block_type=BlockType.PARAGRAPH, confidence=1.0
        )
        with pytest.raises(ValidationError):
            DocumentBlock(
                block_id="1_0", page=1, text="x", block_type=BlockType.PARAGRAPH, confidence=1.1
            )
        with pytest.raises(ValidationError):
            DocumentBlock(
                block_id="1_0", page=1, text="x", block_type=BlockType.PARAGRAPH, confidence=-0.1
            )
...
class TestDocumentJSON:
    ...
    def test_block_page_bounds(self) -> None:
        ...
        with pytest.raises(
            ValidationError, match="Block 999_0 references page 999 but document only has 10 pages"
        ):
            DocumentJSON(
                document_id="d1",
                blob_id="b1",
    ...
    def test_table_page_bounds(self) -> None:
        ...
        table = ExtractedTable(table_id="t5", page=5, cells=[], row_count=0, col_count=0)
        DocumentJSON(
            document_id="d1",
            blob_id="b1",
            file_hash="h",
        ...
        with pytest.raises(
            ValidationError, match="Table t999 references page 999 but document only has 10 pages"
        ):
            DocumentJSON(
                document_id="d1",
                blob_id="b1",
